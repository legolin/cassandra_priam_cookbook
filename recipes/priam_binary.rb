...
# Priam's agent jar 
src_url = node[:cassandra][:priam][:binary][:cass_extensions_jar][:src_url]
local_archive = "#{node[:cassandra][:priam][:binary][:cass_home]}/lib/#{::File.basename src_url}"
remote_file local_archive do
  source  src_url
  mode    0644
  not_if  { File.exists? local_archive }
  checksum node[:cassandra][:priam][:binary][:cass_extensions_jar][:checksum]
end

# Give priam running as node[:tomcat][:user] access to write the Cassandra config
file "#{node[:cassandra][:priam][:binary][:cass_home]}/conf/cassandra.yaml" do
...
bash "Setup Agent in Cassandra Include File" do
  ...
  cwd "/"
  code <<-EOH
  cp #{node[:cassandra][:priam][:binary][:cass_home]}/bin/cassandra.in.sh /tmp
  ...
  cp /tmp/cassandra.in.sh #{node[:cassandra][:priam][:binary][:cass_home]}/
  EOH
